...
class AlbertImgEmbeddings(nn.Module):
    """ Albert Language - Image Embedding
    ...
    def __init__(self, config):
        ...
        self.img_embeddings = nn.Linear(self.img_dim, self.config.embedding_size, bias=True)
        self.position_embeddings = nn.Embedding(config.max_position_embeddings, config.embedding_size)
        self.token_type_embeddings = nn.Embedding(config.type_vocab_size, config.embedding_size)
...
class AlbertLayerGroup(nn.Module):
    ...
    def forward(
       ...
       for layer_index, albert_layer in enumerate(self.albert_layers):
            hidden_states = albert_layer(hidden_states, attention_mask,head_mask, history_state)
       return hidden_states
...
class AlbertEncoder(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, head_mask=None,
        ...
        # Step 1: Project embedding_size → hidden_size
        # e.g. from [batch, seq_len, 128] → [batch, seq_len, 768]
...
class AlbertIFPredictionHead(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        #return hidden_states  # outputs, (hidden states), (attentions)
...
class AlbertPreTrainedModel(PreTrainedModel):
    ...
    def __init__(self, *inputs, **kwargs):
        super(AlbertPreTrainedModel, self).__init__(*inputs, **kwargs)
...
class AlbertModel(AlbertPreTrainedModel):
    ...
    Examples::
        ...
        >>> config = BertConfig.from_pretrained('albert-base-uncased')
...
class AlbertImgModel(AlbertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        if config.img_feature_type == 'dis_code':
            ...
            self.img_embedding = nn.Linear(config.code_dim, self.config.embedding_size, bias=True) #TODO: change to hidden_size
        elif config.img_feature_type == 'dis_code_t': # transpose
            self.code_embeddings = nn.Embedding(config.code_voc, config.code_dim, padding_idx=0)
            self.img_embedding = nn.Linear(config.code_size, self.config.embedding_size, bias=True) #TODO: change to hidden_size
            ...
            self.img_embedding = nn.Linear(self.img_dim, self.config.embedding_size, bias=True) #TODO: change to hidden_size
            self.dropout = nn.Dropout(config.hidden_dropout_prob)
            if self.use_img_layernorm:
                img_norm_eps = getattr(config, 'img_layer_norm_eps', config.layer_norm_eps)
                self.LayerNorm = LayerNormClass(config.embedding_size, eps=img_norm_eps) #TODO: change to hidden_size
