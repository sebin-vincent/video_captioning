...
def find_pruneable_heads_and_indices(
    ...
    Args:
        heads (`List[int]`): List of the heads to prune.
...
def apply_chunking_to_forward(
    forward_fn: Callable[..., torch.Tensor], chunk_size: int, chunk_dim: int, *args, **kwargs
) -> torch.Tensor:
    """
    This function chunks the `input_tensors` into smaller input tensor parts of size `chunk_size` over the `chunk_dim`
    dimension and successively applies the `forward_fn` to each part.

    This is useful for forwarding models that require a lot of memory saving sequential memory allocation.

    Args:
        forward_fn (`Callable[..., torch.Tensor]`):
            The forward function of the model.
        chunk_size (`int`):
            The chunk size of a chunked tensor: `num_chunks = tensor.shape[chunk_dim] / chunk_size`.
        chunk_dim (`int`):
            The dimension over which the `input_tensors` should be chunked.
        *args:
            The positional arguments of the `forward_fn`.
        **kwargs:
            The keyword arguments of the `forward_fn`.

    Returns:
        `torch.Tensor`: A tensor with the same shape as the expected output of `forward_fn`.
    """
    assert len(args) > 0 or len(kwargs) > 0, "You need to pass either args or kwargs"
    if len(args) > 0:
        tensor = args[0]
        if "hidden_states" in kwargs:
            raise ValueError(
                "If `args` are provided, `hidden_states` shouldn't be in `kwargs`. Use `args` instead."
            )
    else:
        tensor = kwargs["hidden_states"]

    # inspect.signature(forward_fn) would be ideal, but it doesn't work with JIT compiled code.
    # The main idea here is to avoid providing the same argument in both `args` and `kwargs`
    # (possible with `hidden_states`), but this is not robust.
    if "hidden_states" in kwargs and len(args) > 0:
        raise ValueError(
            "If `args` are provided, `hidden_states` shouldn't be in `kwargs`. Use `args` instead."
        )

    tensor_shape = tensor.shape[chunk_dim]
    assert tensor_shape % chunk_size == 0, "The tensor has to be divisible by `chunk_size`"

    # inspect.signature(forward_fn) would be ideal, but it doesn't work with JIT compiled code.
    # The main idea here is to avoid providing the same argument in both `args` and `kwargs`
    # (possible with `hidden_states`), but this is not robust.
    if "hidden_states" in kwargs:
        kwargs_without_hidden_states = {key: value for key, value in kwargs.items() if key != "hidden_states"}

    if len(args) > 0:
        # `args[0]` is `hidden_states`, `args[1:]` is the rest
        rest_of_args = args[1:]
        all_args = (None, *rest_of_args)
    else:
        all_args = (None,)

    output_chunks = []
    for chunked_hidden_states in tensor.split(chunk_size, dim=chunk_dim):
        if len(args) > 0:
            all_args = (chunked_hidden_states, *rest_of_args)
            output_chunk = forward_fn(*all_args)
        else:
            output_chunk = forward_fn(hidden_states=chunked_hidden_states, **kwargs_without_hidden_states)
