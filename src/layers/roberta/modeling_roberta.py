...
"""PyTorch RoBERTa model, similar to BERT but with some architectural differences."""
...
try:
    from apex.normalization.fused_layer_norm import FusedLayerNorm as LayerNormClass
except ImportError:
    logger.info("Better speed can be achieved with apex installed from https://www.github.com/nvidia/apex .")
    from torch.nn import LayerNorm as LayerNormClass
...
class RobertaModel(RobertaPreTrainedModel):
    ...
    Outputs: `Tuple` comprising various elements depending on the configuration (config) and inputs:
        ...
        **pooler_output**: ``torch.FloatTensor`` of shape ``(batch_size, hidden_size)``
            ...
            layer weights are trained from the next sentence prediction (classification) objective during RoBERTa pretraining. This output is usually *not* a good summary
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, position_ids=None, head_mask=None):
        ...
        # used in OpenAI GPT, we just need to prepare the attention mask for the transformer here.
...
# Placeholder classes for other RoBERTa variants
class RobertaForImageCaptioning(RobertaPreTrainedModel):
    """RoBERTa for Image Captioning."""
    def __init__(self, config):
        super(RobertaForImageCaptioning, self).__init__(config)
        self.config = config
        self.roberta = RobertaImgModel(config)
        # Add captioning heads here
        self.apply(self.init_weights)
...
class RobertaImgForPreTraining(RobertaPreTrainedModel):
    pass

class RobertaForVLGrounding(RobertaPreTrainedModel):
    pass

class RobertaImgForGroundedPreTraining(RobertaPreTrainedModel):
    pass
