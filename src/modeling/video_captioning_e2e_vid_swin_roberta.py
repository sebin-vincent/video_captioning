...
class VideoTransformerRoBERTa(torch.nn.Module):
    def __init__(self, args, config, swin, transformer_encoder):
        super(VideoTransformerRoBERTa, self).__init__()
        self.config = config
    ...
    def forward(self, *args, **kwargs):
        ...
        if self.trans_encoder.roberta.encoder.output_attentions:
            self.trans_encoder.roberta.encoder.set_output_attentions(False)
        # learn soft attention mask
    ...
    def diag_based_init_attn_mask(self, pretrain_attn_mask):
        # Initialize attention mask based on pretrained mask
        pass

    def freeze_backbone(self, freeze=True):
        """Freeze the Swin backbone."""
